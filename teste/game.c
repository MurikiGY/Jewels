...
SPRITES sprites;    //CRIAÇÃO DA VARIAVEL SPRITES
...
void ship_update()
{
    ...
    {   //Se levou tiro
        if(shots_collide(true, ship.x, ship.y, SHIP_W, SHIP_H))
        {
            ...
            ship.lives--;
            ship.respawn_timer = 90;      //Tempo em quadros
            ship.invincible_timer = 180;  //Tempo em quadros
        }
    }
...
void aliens_update()
{
    //Numero de aliens que aparecem a cada 120 frames
    int new_quota =
        (frames % 120)
        ? 0
        : between(2, 4)
    ...
    for(int i = 0; i < ALIENS_N; i++)
    {
        //Busca por alien no vetor não usado
        if(!aliens[i].used)
        {
        ...
        //Se alien chegar no limite do display
...
int main()
{
    ...
    //Inicia display e audio
    disp_init();

    //Inicia variaveis globais de audio
    audio_init();
    ...
    //Inicia teclado, fx, tiros, naves...
    keyboard_init();    //Zera vetor de teclado
    fx_init();          //fx[i].used = false;
    shots_init();       //shots[i].used = false
    ship_init();        //Atribui valores a ship
    aliens_init();      //aliens[i].used = false
    stars_init();       //Inicia estrelas
    ...
    while(1)
    {
        ...
        {
            case ALLEGRO_EVENT_TIMER:
                fx_update();      //Itera sobre FX[]
                shots_update();   //Itera sobre shots[]
                stars_update();   //Itera sobre stars[]
                ship_update();
                aliens_update();
                hud_update();
